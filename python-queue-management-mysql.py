...
# Connect to MySQL database on Computer B (server)
db_config = {
    'host': '192.AAA.BCC.YY',  # IP address of Computer B if not running locally
    'user': 'inputuser',
    'password': 'inputpassword',
    'database': 'databaseofmysqlname'  # Replace with your actual database name
}
...
# SQL Script
cursor.execute('''
CREATE TABLE IF NOT EXISTS table-queue-management (
    obj_id INT,
    timestamps FLOAT,
    x1 INT,
    y1 INT,
...
# Clear the table
cursor.execute('DELETE FROM table-queue-management')
conn.commit()
...
video_writer = cv2.VideoWriter("resultofqueue-mysql.mp4", cv2.VideoWriter_fourcc(*"mp4v"), fps, (w, h))
                ...
                for i, box in enumerate(boxes):
                    ...
                    if result.names[cls_id] == 'person' and obj_id not in seen_ids:
                        ...
                        if is_point_in_polygon(center_x, center_y, queue_region):
                            ...
                            cursor.execute('''
                            INSERT INTO table-queue-management (obj_id, timestamps, x1, y1, x2, y2) VALUES (%s, %s, %s, %s, %s, %s)
                            ''', (obj_id, timestamp, x1, y1, x2, y2))
                            conn.commit()
