...
# Kafka configuration
kafka_conf = {
    'bootstrap.servers': '192.AAA.BCC.YY:XXXX',  # IP address of Computer server that running Kafka
}
producer = Producer(kafka_conf)

def delivery_report(err, msg):
    """Delivery report callback to confirm successful sending or capture errors."""
    if err is not None:
...
video_writer = cv2.VideoWriter("resultofqueue-kafka.mp4", cv2.VideoWriter_fourcc(*"mp4v"), fps, (w, h))
                ...
                for i, box in enumerate(boxes):
                    ...
                    if result.names[cls_id] == 'person' and obj_id not in seen_ids:
                        ...
                        if is_point_in_polygon(center_x, center_y, queue_region):
                            ...
                            send_to_kafka('table-queue-management', key=str(obj_id), value=str(message))
